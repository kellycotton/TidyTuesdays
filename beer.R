...
plot_usmap(data = beer_tuition, values = "mean_barrels") + 
  scale_fill_viridis(name="Mean Barrels Produced",label = scales::comma) + 
  theme(legend.position = "right") + ggtitle("Average Annual Barrels Produced")
...
ggplot(kegs_tuition) + aes(x=mean_barrels,y=mean_tuition,color=region) +
  ...
  scale_x_log10(label = scales::comma) +
  scale_color_viridis(discrete = TRUE) +
  labs(y="Mean Annual In-State Tuition ($)",x="Annual Barrels Produced (Kegs)",
...
ggplot(milk_beer) + aes(x=mean_barrels,y=mean_milk,color=region) +
  ...
  scale_color_viridis(discrete=TRUE) +
  labs(y="Annual Milk Produced (Pounds)",x="Annual Barrels of Beer Produced",
